...
public abstract class AbstractEnderLeaves extends Default implements IShearable {
	
    public static final PropertyBool DECAYABLE = PropertyBool.create("decayable");
    public static final PropertyBool CHECK_DECAY = PropertyBool.create("check_decay");
    int[] surroundings;
	
    public AbstractEnderLeaves(String name) {
    	super(Material.LEAVES, name, 0.2F, 10F, true);
        setLightOpacity(1);
                    ...
                    for (int i2 = -4; i2 <= 4; ++i2) {
                        for (int j2 = -4; j2 <= 4; ++j2) {
                            for (int k2 = -4; k2 <= 4; ++k2) {
                                IBlockState iblockstate = world.getBlockState(mpos.setPos(k + i2, l + j2, i1 + k2));
                                Block block = iblockstate.getBlock();

                                if (!block.canSustainLeaves(iblockstate, world, mpos.setPos(k + i2, l + j2, i1 + k2))) {
                                    if (block.isLeaves(iblockstate, world, mpos.setPos(k + i2, l + j2, i1 + k2))) {
    ...
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return Item.getItemFromBlock(ListBlock.ENDER_SAPLING);
    }
    ...
    public abstract EnumType getWoodTypes(int meta);
    ...
    public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
        ...
        if (world instanceof World) this.dropApple((World)world, pos, state, chance);
