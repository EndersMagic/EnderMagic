...
public class EnderLeaves extends AbstractEnderLeaves implements IMetaBlockName {
    ...
    protected void dropApple(World world, BlockPos pos, IBlockState state, int chance) {
        if (state.getValue(TYPE) == EnumType.DRAGON && world.rand.nextInt(chance) == 0) {
            spawnAsEntity(world, pos, new ItemStack(ListItem.ENDER_APPLE));
        }
    }
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(TYPE, this.getWoodTypes(meta)).withProperty(DECAYABLE, Boolean.valueOf((meta & 4) == 0)).withProperty(CHECK_DECAY, Boolean.valueOf((meta & 8) > 0));
    }
	...
	@Override
    public EnumType getWoodTypes(int meta) {return EnumType.values()[(meta & 3) % 4];}
    
	@Override
    public int damageDropped(IBlockState state) {return ((EnderPlanks.EnumType)state.getValue(TYPE)).getID();}
