...
public class TeleportBlock extends BlockContainer {
	...
	public void onEntityWalk(World world, BlockPos pos, Entity entity) {
		...
		if(world.getTileEntity(pos) instanceof TileTPBlock) {
			...
			if(world.getBlockState(pos.up(2)).getBlock() == Blocks.STONE_BUTTON) {
				if(world.getBlockState(pos.up(3)).getBlock() == ListBlock.GEN && ((Integer)world.getBlockState(pos.up(3)).getValue(GeneratorBase.layers)).intValue() > 1) {
								...
								if(flag) {
									if(world.getBlockState(pos.up(2)).getValue(BlockButton.POWERED) == Boolean.valueOf(true) ) {
										if(tile.getToPos() != null) {	
											...
											if(world.getBlockState(toPos) == Blocks.BONE_BLOCK.getDefaultState().withProperty(BlockBone.AXIS, EnumFacing.Axis.Y)) {
												...
												world.setBlockState(pos.up(3), ListBlock.GEN.getDefaultState().withProperty(GeneratorBase.layers, i - 1));
											}
											else {
												int i = world.getBlockState(pos.up(3)).getValue(GeneratorBase.layers);
												world.setBlockState(pos.up(3), ListBlock.GEN.getDefaultState().withProperty(GeneratorBase.layers, i - 1));
