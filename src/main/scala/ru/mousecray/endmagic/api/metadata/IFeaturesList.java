...
public interface IFeaturesList extends IStringSerializable {
    ...
    @Nonnull
    static IFeaturesList EMPTY(String blockName) {
        return () -> blockName;
    }
    ...
    @Nullable
    default TileEntity createTileEntity() {
        return null;
    }
    ...
    default Material getMaterial(IBlockState state) {
        return state.getBlock().getMaterial(state);
    }

    default MapColor getMapColor(IBlockState state, IBlockAccess access, BlockPos pos) {
        return state.getBlock().getMapColor(state, access, pos);
    }

    default float getHardness(IBlockState state, World world, BlockPos pos) {
    ...
    default int getLightOpacity(IBlockState state, IBlockAccess world, BlockPos pos) {
        return state.getBlock().getLightOpacity(state, world, pos);
    }

    @Nullable
    default EnumBlockRenderType getRenderType(IBlockState state) {
        return state.getBlock().getRenderType(state);
    }
    ...
    default boolean canEntitySpawn(IBlockState state, Entity entity) {
        return state.getBlock().canEntitySpawn(state, entity);
    }

    @Nullable
    default AxisAlignedBB getCollisionAABB(IBlockState state, IBlockAccess world, BlockPos pos) {
        return state.getBlock().getCollisionBoundingBox(state, world, pos);
    }

    @Nullable
    default AxisAlignedBB getSelectedAABB(IBlockState state, World world, BlockPos pos) {
    ...
    default SoundType getSoundType(IBlockState state) {
    ...
    default boolean isTopSolid(IBlockState state) {
        return state.getBlock().isTopSolid(state);
    }

    default int quantityDropped(IBlockState state, int fortune, Random rand) {
        return state.getBlock().quantityDropped(state, fortune, rand);
    }
}